...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchMaintenanceReport = async () => {
      const res = await getAllMaintenanceReport();
      setMaintenanceCount(res.length);
    };
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-xl shadow-md flex items-center justify-between hover:shadow-lg transition-all">
            ...
            <div className="p-4 bg-indigo-100 rounded-full">
              <FaBed className="text-indigo-600 w-6 h-6" />
            </div>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-md flex items-center justify-between hover:shadow-lg transition-all">
            <div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-md flex items-center justify-between hover:shadow-lg transition-all">
            <div>
              <p className="text-sm text-gray-500">Maintenance Reports</p>
              <h3 className="text-3xl font-bold text-red-600">
                {maintenanceCount}
              </h3>
            </div>
            <div className="p-4 bg-red-100 rounded-full">
              <FaTools className="text-red-600 w-6 h-6" />
            </div>
          </div>
        </section>
